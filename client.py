...
# transfer files in 64 KB chunks
BUFFER_SIZE = 64 * 1024
...
def receive_exact(connection: socket.socket, byte_count: int) -> bytes:
    ...
    # recv() may return less data than requested, so keep going
    while len(data) < byte_count:
        chunk = connection.recv(
            min(BUFFER_SIZE, byte_count - len(data))
        )

        # an empty result means the connection was closed
        if not chunk:
...
def send_header(connection: socket.socket, header: dict) -> None:
    """send a length-prefixed JSON header."""
    encoded_header = json.dumps(header).encode("utf-8")

    # send the header size first, followed by the actual header
    connection.sendall(
...
def receive_header(connection: socket.socket) -> dict:
    ...
    # the first four bytes tell us how large the header is
    ...
    # receive and decode the actual JSON header
...
def format_size(size: int) -> str:
    """convert bytes into a more readable size."""
    value = float(size)
...
def upload_file(
    ...
    # stop if the file path is invalid
    if not path.is_file():
    ...
    # send the file details before sending the file itself
    send_header(
        connection,
        {
            "type": "upload",
...
def download_file(
    ...
    # stop if the server could not find or send the file
    ...
    # remove any folder information from the filename
    safe_filename = Path(
        response["filename"]
    ).name
    ...
    # save incoming file data into client_files
...
def list_files(connection: socket.socket) -> None:
    """show the files currently stored on the server."""
    send_header(
        connection,
        {
            "type": "list",
...
def delete_file(
    ...
    """delete a file stored on the server."""
    send_header(
        connection,
        {
            "type": "delete",
...
def run_system_command(
    ...
    """send a system information command to the server."""
    send_header(
        connection,
        {
            "type": "command",
...
def main() -> None:
    """connect to the server and start the command loop."""
    ...
    print(
        f"Client files folder: "
        f"{CLIENT_FILES.resolve()}"
    )
    print(
    ...
    try:
        ...
        # remove the timeout after the connection succeeds
        ...
        print(
            "The computer was reachable, but nothing accepted "
        ...
        # always close the socket before the program ends
        client_socket.close()
